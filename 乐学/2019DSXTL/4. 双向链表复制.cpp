...
{
    DuLinkList lis = DHead;
    while (lis->next != DHead)
    {
        lis = lis->next;
    }
    DuLinkList forward, back;
    forward = back = lis;
    while (back->prior != DHead)
    {
        back = back->prior;
        forward->next = (DuLinkList)malloc(sizeof(DuLNode));
        forward->next->prior = forward;
        forward = forward->next;
        forward->data = back->data;
        forward->next = DHead;
        DHead->prior = forward;
    }
}
