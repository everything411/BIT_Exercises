...
int main(int argc, char const *argv[])
{
    ...
    int point_a, point_b, rank;
    while (scanf("%d%d", &point_count, &edge_count) && point_count && edge_count)
    {
        sp.init(point_count, edge_count);
        for (int i = 0; i < edge_count; i++)
        {
            scanf("%d%d%d", &point_a, &point_b, &rank);
            sp.addedge(point_a, point_b, rank);
        }
        printf("%d\n", sp.spfa(1));
        printf("%d\n", sp.dist[point_count]);
    }
...
inline void ShortestPath_t::init(int _N, int _M)
{
    N = _N;
    M = _M;
